...
function RecommendedSection({ data, setData }: IpropsData) {
    ...
    const RecommendedData = data.filter((e: any) => e.isTrending !== true)
                        ...
                        <div>
                            <RecommendedMovie key={index} image={`url(${RecommendedData[index].thumbnail.regular.large})`} >
                                <BookmarkIconDiv onClick={() => {
                                    handleClick(index)
                                    item.isBookmarked = !item.isBookmarked}}><img src={!bookmarked[index] ? emptyBookmark : fullBookmark} onClick={() => setIsActive(!isActive)} alt="bookmark empty icon" /></BookmarkIconDiv>
                            </RecommendedMovie>
